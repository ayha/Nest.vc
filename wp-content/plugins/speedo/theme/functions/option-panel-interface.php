...
/*-----------------------------------------------------------------------------------*/
/* SPEEDO Admin Interface - SPEEDO_add_admin */
/*-----------------------------------------------------------------------------------*/

// Load static framework options pages 
function SPEEDO_add_admin() {

    global $query_string;
    
    $templatename =  get_option('templatename');      
    $the_prefix =  get_option('the_prefix'); 
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'speedo_optionspanel' ) {
		if (isset($_REQUEST['the_save']) && 'reset' == $_REQUEST['the_save']) {
			$varoptions =  get_option('the_template'); 
			SPEEDO_reset_options($varoptions,'speedo_optionspanel');
			header("Location: options-general.php?page=speedo_optionspanel&reset=true");
			die;
		}
    }
		
    $options_page = add_menu_page($templatename, 'Speedo Options', 'edit_theme_options', 'speedo_optionspanel','SPEEDO_options_page', $icon);
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$options_page", 'SPEEDO_load_only');
} 

add_action('admin_menu', 'SPEEDO_add_admin');

/*-----------------------------------------------------------------------------------*/
/* SPEEDO Reset Function - SPEEDO_reset_options */
/*-----------------------------------------------------------------------------------*/
function SPEEDO_reset_options($varoptions,$page = ''){
	...
	foreach($varoptions as $varoption){
		...
		if(isset($varoption['id'])){ 
			$count++;
			$option_id = $varoption['id'];
			$option_type = $varoption['type'];
			...
			if($option_type == 'multicheck'){
				$multicount = 0;
				foreach($varoption['options'] as $option_key => $option_option){
					$multicount++;
	...
	//When Theme Options page is reset - Add the the_options option
	if($page == 'speedo_optionspanel'){
		$query_inner .= " OR option_name = 'the_options'";
	}
...
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - SPEEDO_options_page */
/*-----------------------------------------------------------------------------------*/
function SPEEDO_options_page(){
    $varoptions =  get_option('the_template');      
    $templatename =  get_option('templatename');
?>
...
<div class="wrap" id="of_container">
  ...
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2><?php echo $templatename; ?> Options Panel</h2>
      </div>
    ...
    <?php 
        ...
        $return = SPEEDO_machine($varoptions);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
    ...
    <img style="display:none" src="<?php echo SPEEDO_THEME_DIR; ?>/functions/images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
  ...
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    ...
    <input type="hidden" name="the_save" value="reset" />
    </span>
  </form>
</div>
...
/*-----------------------------------------------------------------------------------*/
/* Load required javascripts for Options Page - SPEEDO_load_only */
/*-----------------------------------------------------------------------------------*/
function SPEEDO_load_only() {

	add_action('admin_head', 'SPEEDO_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_register_script('jquery-input-mask', SPEEDO_THEME_DIR.'/functions/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	
	function SPEEDO_admin_head() { 
			
		echo '<link rel="stylesheet" type="text/css" href="'.SPEEDO_THEME_DIR.'/functions/admin-style.css" media="screen" />';
		
		 // COLOR Picker ?>
<link rel="stylesheet" media="screen" type="text/css" href="<?php echo SPEEDO_THEME_DIR; ?>/functions/css/colorpicker.css" />
<script type="text/javascript" src="<?php echo SPEEDO_THEME_DIR; ?>/functions/js/colorpicker.js"></script>
<script type="text/javascript" language="javascript">
		jQuery(document).ready(function(){
			
			
			//Color Picker
			<?php $varoptions = get_option('the_template');
			
			foreach($varoptions as $varoption){ 
			if($varoption['type'] == 'color' OR $varoption['type'] == 'typography' OR $varoption['type'] == 'border'){
				if($varoption['type'] == 'typography' OR $varoption['type'] == 'border'){
					$option_id = $varoption['id'];
					$temp_color = get_option($option_id);
					$option_id = $varoption['id'] . '_color';
...
<?php
		//AJAX Upload
		?>
<script type="text/javascript" src="<?php echo SPEEDO_THEME_DIR; ?>/functions/js/ajaxupload.js"></script>
<script type="text/javascript">
			...
			new AjaxUpload(clickedID, {
				  ...
				  data: { // Additional data to send
						action: 'speedo_ajax_post_action',
						type: 'upload',
						data: clickedID },
			...
			jQuery('#ofform').submit(function(){
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'speedo_optionspanel'){ ?>
						type: 'options',
						<?php } ?>
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - SPEEDO_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_speedo_ajax_post_action', 'SPEEDO_ajax_callback');

function SPEEDO_ajax_callback() {
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($varoptions as $option_array){
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					elseif($type == 'multicheck'){ // Multi Check Save
						...
						$option_options = $option_array['varoptions'];
...
/*-----------------------------------------------------------------------------------*/
/* Generates The Options Within the Panel - SPEEDO_machine */
/*-----------------------------------------------------------------------------------*/
function SPEEDO_machine($varoptions) {
        
    $counter = 0;
	$menu = '';
	$output = '';
	foreach ($varoptions as $value) {
		...
		case 'select':
			...
			foreach ($value['options'] as $varoption) {
				
				$selected = '';
				
				 if($select_value != '') {
					 if ( $select_value == $varoption) { $selected = ' selected="selected"';} 
			     } else {
					 if ( isset($value['std']) )
						 if ($value['std'] == $varoption) { $selected = ' selected="selected"'; }
				 }
		...
		case 'select2':
			...
			foreach ($value['options'] as $varoption => $name) {
				...
				$selected = '';
				 ...
				 $output .= '<option'. $selected .' value="'.$varoption.'">';
				 $output .= $name;
				 $output .= '</option>';
		...
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				$output .= '<textarea class="of-input" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="6">'.$ta_value.'</textarea>';
		...
		case "radio":
			 ...
			 foreach ($value['options'] as $key => $varoption) 
			 { 
				...
				$output .= '<input class="of-input of-radio" type="radio" name="'. $value['id'] .'" value="'. $key .'" '. $checked .' />' . $varoption .'<br />';
		...
		case "multicheck":
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox of-input" name="'. $of_key .'" id="'. $of_key .'" value="true" '. $checked .' /><label for="'. $of_key .'">'. $varoption .'</label><br />';
										
			}
		break;
		case "upload":
			
			$output .= SPEEDO_uploader_function($value['id'],$value['std'],null);
			
		break;
		case "upload_min":
			
			$output .= SPEEDO_uploader_function($value['id'],$value['std'],'min');
		...
		case "images":
			...
			foreach ($value['options'] as $key => $varoption) 
			 ...
			 $i++;
				...
				$output .= '<img src="'.$varoption.'" alt="" class="of-radio-img-img '. $selected .'" onClick="document.getElementById(\'of-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
				$output .= '</span>';
		...
		case "note":
			$output .= '<div class="note"><h3>'.$value['title'].'</h3>'.'<p>'.$value['msg'].'</p>'.'</div>'."\n";
		break;
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - SPEEDO_uploader_function */
/*-----------------------------------------------------------------------------------*/
function SPEEDO_uploader_function($id,$std,$mod){
